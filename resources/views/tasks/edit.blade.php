...
@section('content')
    <div class="form-container">
        ...
        <form action="{{ route('tasks.update', $task->id) }}" method="POST" class="note-form" id="edit-task-form">
            @csrf
            @method('PUT')
            ...
            <div style="background: white; border-radius: var(--radius-lg); padding: 30px; box-shadow: var(--shadow-md); border: 1px solid var(--color-border); display: flex; flex-direction: column; gap: 24px;">
                ...
                <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 20px;">
                    ...
                    <div class="form-group">
                        ...
                        <input type="color" name="color" id="color" class="form-input @error('color') is-error @enderror" value="{{ old('color', $task->color ?? '#547792') }}" style="height: 45px; cursor: pointer; padding: 5px;">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                    ...
                    <div class="form-group">
                        <label for="status" class="form-label">
                            <span class="label-icon">📊</span>
                            Status Saat Ini <span class="required">*</span>
                        </label>
                ...
                <div style="padding: 20px; background: rgba(148, 180, 193, 0.05); border-radius: var(--radius-md); border: 1px dashed var(--color-secondary);">
                    <div class="form-group" style="margin-bottom: 0;">
                        ...
                        <select name="assigned_user_id" id="assigned_user_id" class="form-input @error('assigned_user_id') is-error @enderror">
                            <option value="">-- Diri Sendiri / Lepas Penugasan --</option>
                            @foreach($users as $u)
                                <option value="{{ $u->id }}" {{ old('assigned_user_id', $task->assigned_user_id) == $u->id ? 'selected' : '' }}>
                                    {{ $u->name }} ({{ $u->email }})
                                </option>
                            @endforeach
                        </select>
                        <p class="form-subtitle" style="font-size: 0.75rem; margin-top: 8px; color: var(--color-primary);">Mengubah pelaksana akan memindahkan visibilitas tugas ini ke dashboard rekan tim yang baru.</p>
                ...
                <div class="form-group">
                    ...
                    <textarea name="content" id="content" class="form-input @error('content') is-error @enderror" rows="6">{{ old('content', $task->content) }}</textarea>
